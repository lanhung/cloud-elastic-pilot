...
RUN set -eux; \
    mkdir -p /out; \
    for bin in hooke-ingester hooke-controller hooke-node-agent hooke-correlator hooke-ack-adapter hooke-migrate hookectl smoke-app keda-redis-app e05-gang-worker; do \
