...
REPOSITORY="${E08_IMAGE_REPOSITORY:-hooke/e08}"
PLATFORM="${E08_IMAGE_PLATFORM:-linux/amd64}"
TAG=""
METADATA_FILE=""
PUSH=false
ALLOW_DIRTY=false
...
usage() {
  cat <<'USAGE'
Usage: build-e08-image.sh [options]

Build the E08 collector stack and finite workload image. The image remains
local unless --push is explicitly supplied.

Options:
  --repository REPO   Image repository (default: hooke/e08)
...
if [[ -n "$(git status --porcelain --untracked-files=normal)" ]]; then
  ...
  [[ "$PUSH" == false ]] || {
    echo "refusing to push an E08 image built from a dirty worktree" >&2
    exit 1
  }
  TAG_SUFFIX="-dirty"
fi

: "${TAG:=e08-${GIT_SHORT}${TAG_SUFFIX}}"
[[ "$TAG" != *[[:space:]]* && "$TAG" != *:* && "$TAG" != *@* ]] || {
...
echo "Building E08 collector overhead image: ${LOCAL_REF}" >&2
docker build --platform "$PLATFORM" \
  --provenance=false \
  --sbom=false \
  --file examples/e08-collector-overhead/Dockerfile \
  --tag "$LOCAL_REF" \
  --build-arg "VERSION=e08-${GIT_SHORT}" \
...
[[ "$LABEL_COMMIT" == "$GIT_COMMIT" ]] || {
  echo "E08 image revision label does not match the source commit" >&2
  exit 1
}
...
if [[ "$PUSH" == true ]]; then
  ...
  [[ "$IMMUTABLE_REF" =~ @sha256:[0-9a-f]{64}$ ]] || {
    echo "push completed but the immutable E08 repository digest was not resolved" >&2
    exit 1
  }
fi

emit_metadata() {
  printf 'E08_IMAGE_BUILD_COMMIT=%q\n' "$GIT_COMMIT"
  printf 'E08_IMAGE_SOURCE_STATE=%q\n' "$SOURCE_STATE"
  printf 'E08_IMAGE_PLATFORM=%q\n' "$PLATFORM"
  printf 'E08_LOCAL_REF=%q\n' "$LOCAL_REF"
  printf 'E08_IMAGE_ID=%q\n' "$IMAGE_ID"
  printf 'E08_IMAGE_SIZE_BYTES=%q\n' "$IMAGE_SIZE_BYTES"
  printf 'E08_GO_BUILD_BASE=%q\n' "golang:1.23-bookworm@sha256:167053a2bb901972bf2c1611f8f52c44d5fe7e762e5cab213708d82c421614db"
  printf 'E08_RUNTIME_BASE=%q\n' "gcr.io/distroless/static-debian12:nonroot@sha256:f5b485ea962d9bd1186b2f6b3a061191539b905b82ec395de78cbfae51f20e35"
  if [[ "$PUSH" == true ]]; then
    printf 'E08_IMAGE=%q\n' "$IMMUTABLE_REF"
  fi
}
...
if [[ -n "$METADATA_FILE" ]]; then
  ...
  echo "E08 image metadata: ${METADATA_FILE}" >&2
else
  emit_metadata
fi
