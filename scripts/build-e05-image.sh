...
REPOSITORY="${E05_IMAGE_REPOSITORY:-hooke/e05}"
PLATFORM="${E05_IMAGE_PLATFORM:-linux/amd64}"
TAG=""
METADATA_FILE=""
PUSH=false
ALLOW_DIRTY=false
...
usage() {
  cat <<'USAGE'
Usage: build-e05-image.sh [options]

Build the E05 ACK gang worker image. The image remains local unless --push is
explicitly supplied.

Options:
  --repository REPO   Image repository (default: hooke/e05)
...
if [[ -n "$(git status --porcelain --untracked-files=normal)" ]]; then
  ...
  [[ "$PUSH" == false ]] || {
    echo "refusing to push an E05 image built from a dirty worktree" >&2
    exit 1
  }
  TAG_SUFFIX="-dirty"
fi

: "${TAG:=e05-${GIT_SHORT}${TAG_SUFFIX}}"
[[ "$TAG" != *[[:space:]]* && "$TAG" != *:* && "$TAG" != *@* ]] || {
...
echo "Building E05 ACK gang worker image: ${LOCAL_REF}" >&2
docker build --platform "$PLATFORM" \
  --provenance=false \
  --sbom=false \
  --file examples/e05-ack-gang/Dockerfile \
  --tag "$LOCAL_REF" \
  --build-arg "VERSION=e05-${GIT_SHORT}" \
...
[[ "$LABEL_COMMIT" == "$GIT_COMMIT" ]] || {
  echo "E05 image revision label does not match the source commit" >&2
  exit 1
}
...
if [[ "$PUSH" == true ]]; then
  ...
  [[ "$IMMUTABLE_REF" =~ @sha256:[0-9a-f]{64}$ ]] || {
    echo "push completed but the immutable E05 repository digest was not resolved" >&2
    exit 1
  }
fi

emit_metadata() {
  printf 'E05_APP_IMAGE_BUILD_COMMIT=%q\n' "$GIT_COMMIT"
  printf 'E05_APP_IMAGE_SOURCE_STATE=%q\n' "$SOURCE_STATE"
  printf 'E05_APP_IMAGE_PLATFORM=%q\n' "$PLATFORM"
  printf 'E05_APP_LOCAL_REF=%q\n' "$LOCAL_REF"
  printf 'E05_APP_IMAGE_ID=%q\n' "$IMAGE_ID"
  printf 'E05_APP_IMAGE_SIZE_BYTES=%q\n' "$IMAGE_SIZE_BYTES"
  printf 'E05_GO_BUILD_BASE=%q\n' "golang:1.23-bookworm@sha256:167053a2bb901972bf2c1611f8f52c44d5fe7e762e5cab213708d82c421614db"
  printf 'E05_RUNTIME_BASE=%q\n' "gcr.io/distroless/static-debian12:nonroot@sha256:f5b485ea962d9bd1186b2f6b3a061191539b905b82ec395de78cbfae51f20e35"
  if [[ "$PUSH" == true ]]; then
    printf 'E05_APP_IMAGE=%q\n' "$IMMUTABLE_REF"
  fi
}
...
if [[ -n "$METADATA_FILE" ]]; then
  ...
  echo "E05 image metadata: ${METADATA_FILE}" >&2
else
  emit_metadata
fi
