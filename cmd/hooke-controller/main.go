...
func main() {
	...
	batcher.Start(ctx)
	defer batcher.Close()
	collector, err := kube.NewCollector(kube.Config{ClusterID: clusterID, DefaultRunID: config.String("HOOKE_ACTIVE_RUN_ID", ""), HookeNamespace: config.String("HOOKE_NAMESPACE", "hooke-system"), CaptureUnlabeled: config.Bool("HOOKE_CAPTURE_UNLABELED", false), Kubeconfig: config.String("KUBECONFIG", "")}, batcher, logger)
	if err != nil {
