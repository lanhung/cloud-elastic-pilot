...
func main() {
	...
	batcher.Start(ctx)
	defer batcher.Close()
	sampler, err := transport.NewSamplingEmitter(batcher, config.Int("HOOKE_EVENT_SAMPLE_PERCENT", 100))
	if err != nil {
		logger.Error("sampling configuration", "error", err)
		os.Exit(2)
	}
	collector, err := kube.NewCollector(kube.Config{ClusterID: clusterID, DefaultRunID: config.String("HOOKE_ACTIVE_RUN_ID", ""), HookeNamespace: config.String("HOOKE_NAMESPACE", "hooke-system"), WatchActiveRunConfigMap: config.Bool("HOOKE_WATCH_ACTIVE_RUN_CONFIGMAP", true), CaptureUnlabeled: config.Bool("HOOKE_CAPTURE_UNLABELED", false), Kubeconfig: config.String("KUBECONFIG", "")}, sampler, logger)
	if err != nil {
		logger.Error("create collector", "error", err)
		os.Exit(1)
	}
	mux := http.NewServeMux()
	observability.RegisterCommon(mux, collector.Ready)
