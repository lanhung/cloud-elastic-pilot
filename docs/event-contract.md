...
每条消息使用 run 内唯一的 `message_id`。producer 仅在 Redis `RPUSH` 成功返回后
写出 `MESSAGE_ENQUEUED`，worker 仅在 `BLPOP` 成功返回后写出
`MESSAGE_DEQUEUED`；失败的 Redis 操作不能生成成功事件。应用进程在边界处写入
Epoch 纳秒 `source_time_ns`，日志导出器再用冻结的 Pod UID、Container ID、
Node、cluster 和 run 时间窗校验关联。

队列深度、KEDA external metric、ScaledObject Active/Inactive、HPA desired/current
和 Deployment scale-to-zero 是控制链路观察。它们必须保存各自的 source time 和
observed time；API 轮询或 status watch 不能冒充 Redis 内部采样或应用执行时刻。
ScaledObject condition 缺少有效 transition time 时必须
`approximate=true`，并写明后备精度。详细 Gate 见
[`e04-keda-scale-to-zero.md`](e04-keda-scale-to-zero.md)。
