...
| Workflow critical path | Argo CR/Node/Edge 采集、确定性拓扑排序、弹性乘积及 MySQL metric 接线已实现 | 是（待 E06 冒烟） | 非法/空图 fail-closed；按 Workflow UID 保存边和 predicted/measured/error |
...
| E06 Argo Workflow 编排 | 1×2 配对冒烟 runner、stage worker、ACK 3.5.15 预检与 Gate 已实现，待集群执行 | 是（待冒烟） | baseline 串行、tuned 并行；业务真实依赖冻结，B/C 重叠、12/12 应用事件、关键路径 6→5 和固定节点为硬 Gate |
...
采样器或业务埋点；E06 虽已补齐 Argo CR/Edge、应用事件和编排代码，仍需真实 ACK
冒烟证据。工程不会用配置字段或假数据替代真实准入、barrier 或 Workflow 事件。
